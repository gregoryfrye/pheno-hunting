...
export async function getStrains() {
  if (!notion || !STRAINS_DB) return [];
  const res = await notion.databases.query({ database_id: STRAINS_DB });
...
export async function saveLog(data) {
  ...
  const date = data.date
    ?? (data.savedAt ? data.savedAt.split("T")[0] : new Date().toISOString().split("T")[0]);

  const fallbackTitle = `${data.plantName || "Plant"} — ${new Date(date).toLocaleDateString("en-US", { month: "short", day: "numeric" })}`;
  const logTitle = data.name || fallbackTitle;

  const plantPageId = data.plantName
    ? await resolvePlantPageId(data.plantName)
    : null;

  // Concern: prefer array from data.concerns; fall back to comma-split data.issues
  const concernItems = Array.isArray(data.concerns) && data.concerns.length
    ? data.concerns.filter(s => s && s !== "None").map(s => ({ name: s }))
    : data.issues
      ? data.issues.split(",").map(s => s.trim()).filter(Boolean).map(s => ({ name: s }))
      : [];

  // CTA: array of { name }
  const ctaItems = Array.isArray(data.cta) && data.cta.length
    ? data.cta.map(s => ({ name: s }))
    : [];
  ...
  const page = await notion.pages.create({
    ...
    properties: {
      ...
      ...(concernItems.length ? { Concern: { multi_select: concernItems } }                                  : {}),
      ...(ctaItems.length    ? { CTA:     { multi_select: ctaItems } }                                       : {}),
